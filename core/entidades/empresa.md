...
- **Nomenclatura (banco + código):** inglês — tabela `companies`, entidade `Company`; colunas `name`, `phone`, `address_*`, `is_active` (boolean, ativo/inativo); `cnpj` mantido como identificador brasileiro. Domínio/documentos continuam em PT.
...
- **CRUD IMPLEMENTADO (2026-09).** `CompaniesService` + `CompaniesController` + DTOs (`create-company.dto`, `update-company.dto`) criados e registrados no `CompaniesModule`. Rotas: `POST/GET /companies`, `GET/PATCH/DELETE /companies/:id`. Soft delete (`isActive: false`), validação `class-validator`, rotas raiz sem prefixo. `cnpj` somente 14 dígitos (validação de dígito verificador = pendente, Camada 1).
